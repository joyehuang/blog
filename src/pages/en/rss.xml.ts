...
const imagesGlob = import.meta.glob<{ default: ImageMetadata }>(
  '/src/content/blog-en/**/*.{jpeg,jpg,png,gif}'
)

const renderContent = async (post: CollectionEntry<'blogEn'>, site: URL) => {
    ...
    return async function (tree: Root) {
      ...
      visit(tree, 'image', (node) => {
        if (node.url.startsWith('/images')) {
          ...
          const imagePathPrefix = `/src/content/blog-en/${post.id}/${node.url.replace('./', '')}`
...
const GET = async (context: AstroGlobal) => {
  const allPostsByDate = sortMDByDate(
    (await getBlogCollection('blogEn')) as CollectionEntry<'blogEn'>[]
  ...
  return rss({
    ...
    description: 'English posts feed',
